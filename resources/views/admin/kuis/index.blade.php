<x-layout.admin.app>
    ...
    <div class="row">
        <div class="col-md-12">
            <x-card.card>
                <x-card.header>
                    <x-button.btn url="{{ url('admin/kuis/tambah') }}" class="success float-end" label="Tambah data" />
                </x-card.header>
                <x-card.body>
                    <x-table.table>
                        <x-table.thead>
                        ...
                        <x-table.tbody>
                            @foreach ($list as $item)
                                <tr>
                                    ...
                                    <x-table.td-action>
                                        <x-button.btn class="warning" icons icon="mdi-eye"
                                            url="{{ url('admin/kuis/detail', $item->id) }}" />
                                        <x-button.btn class="primary" icons icon="mdi-pencil"
                                            url="{{ url('admin/kuis/update', $item->id) }}" />
                                ...
                                <x-modal.modal-delete id="hapus{{ $item->id }}"
                                    action="{{ url('admin/kuis/delete', $item->id) }}" />
                                <!--- #Modal hapus --->
                            @endforeach
                        </x-table.tbody>
                    </x-table.table>
