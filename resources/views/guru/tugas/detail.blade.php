                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <ul class="ul-detail">
                                ...
                                <x-list.li-horizontal left="MAPEL" right="{{ $list->mapel->mapel }}" />
                                <x-list.li-horizontal left="JUDUL TUGAS" right="{{ $list->nama_tugas }}" />
                                <x-list.li-horizontal left="SOAL" right="{{ $list->soal_tugas }}" />
                        ...
                        <div class="col-md-6">
                            <ul class="ul-detail">
                                <x-list.li-horizontal left="KETERANGAN" right="{{ $list->selisihTanggal() }}"
                                    class="{{ $list->selisihTanggal() == 'Tugas sudah berakhir' ? 'text-danger' : '' }} " />
                                ...
                                <x-table.tbody>
                                    @foreach ($list->nilai as $item)
                                        ...
                                        <x-modal.modal-delete id="hapus{{ $item->id }}"
                                            action="{{ url('guru/tugas/delete/nilai', $item->id) }}" />
                                        <!--- #Modal hapus --->

                                        <!--- Modal edit --->
                                        <x-modal.modal id="edit{{ $item->id }}" title="EDIT DATA JAWABAN SISWA"
                                            action="{{ url('guru/tugas/update/nilai', $item->id) }}">
                                            <div class="row">
                                                <div class="col-md-12">
